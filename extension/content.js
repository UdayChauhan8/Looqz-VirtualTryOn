...
function updateMainScreenState() {
  ...
  if (STATE.userPhotoBase64 && STATE.productImageUrl) {
    ...
    if (STATE.creditsRemaining === "0" || STATE.creditsRemaining === 0) {
      ...
      document.getElementById('looqz-try-buy-more').onclick = () => window.open('https://looqz.in/credits');
...
function bindEvents() {
  ...
  document.getElementById('looqz-my-tryons').addEventListener('click', () => window.open('https://looqz.in/my-tryons'));
  document.getElementById('looqz-get-key').addEventListener('click', () => window.open('https://looqz.in/developer'));
  ...
  document.getElementById('looqz-link-banner-buy').addEventListener('click', () => window.open('https://looqz.in/credits'));
...
async function fireTryOn() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 402) {
        errEl.innerHTML = `Credits exhausted. <span class="looqz-text-link" onclick="window.open('https://looqz.in/credits')">Buy more</span>`;
